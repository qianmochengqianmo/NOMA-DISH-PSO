...
if __name__ == "__main__":
    ...
    for Xi in Xi_values:
        for lam1_dB in SNR_dB_list:
            ...
            # 存储
            label = f"Xi={Xi}, SNR={lam1_dB}"
            all_results[label] = np.array(outages_vs_kai)

            key_name = f"Xi_{str(Xi).replace('.', '_')}_SNR_{lam1_dB}"
    ...
    plt.title(f"Outage Probability vs kai11 (M={M}) — 9 Curves")
    plt.xlabel("kai11")
    ...
    # =============== 保存 MAT 文件 =====================
    savemat("outage_kai_SNR_Xi_9curves.mat", export_data)
