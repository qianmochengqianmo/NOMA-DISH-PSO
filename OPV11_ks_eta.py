...
if __name__ == "__main__":
    ...
    # 用户数（建议先用 M=3 验证，之后再改成 9）
    M = 9

    # 固定 SNR（dB），你可以改成 15 或 35
    lam1_dB = 25

    # 横坐标：V11 从 0.01 到 0.1
    V11_grid = np.linspace(0.01, 0.10, 20)

    # ks 和 eta 的组合：3 × 3 = 9 条曲线
    ks_values = [0.001, 0.05, 0.1]
    eta_values = [0.55, 0.65, 0.75]
    ...
    for ks in ks_values:
        ...
        for eta in eta_values:
            ...
            for V11 in V11_grid:
                ...
                outages_vs_V11.append(outage)

            label = f"ks={ks}, eta={eta}"
            all_results[label] = np.array(outages_vs_V11)

            # 导出键名：方便后续在 MATLAB 里按 ks/eta 区分
            key_name = f"ks_{str(ks).replace('.', '_')}_eta_{str(eta).replace('.', '_')}"
            export_data[key_name] = np.array(outages_vs_V11)

    # =============== 画一张图：9 条曲线 =================
    plt.figure(figsize=(9, 7))
    for label, outages in all_results.items():
        plt.plot(V11_grid, outages, marker='o', label=label)

    plt.title(f"Outage Probability vs V11 (kai11={kai11}, Xi={Xi}, SNR={lam1_dB} dB, M={M})")
    plt.xlabel("V11")
    ...
    # =============== 保存 MAT 文件 =================
    savemat("outage_vs_V11_ks_eta_9curves.mat", export_data)
